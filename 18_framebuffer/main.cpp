...
Cube gCube; // for the light source
...
static void updateUniforms()
{
    ...
    glUniform1i(glGetUniformLocation(gScreenShaderProgram.id, "uScreenTexture"), 0); // texture 0
}

// called once every frame during main loop
static void draw()
{
    // clear the "screen" (drawing to our framebuffer)
    GLfloat r = 0.2F; // red
    GLfloat g = 0.3F; // green
    GLfloat b = 0.3F; // blue
    GLfloat a = 1.0F; // alpha
    glBindFramebuffer(GL_FRAMEBUFFER, gFrameBuffer.id); // bind our own frame texture to draw to
    ...
    // second drawing pass - bind the default framebuffer
    glPolygonMode(GL_FRONT_AND_BACK, gScreenTexture.renderMode);
    glActiveTexture(GL_TEXTURE0);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glDisable(GL_DEPTH_TEST); // just a 2d texture
    glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT);

    glUseProgram(gScreenShaderProgram.id);
    glBindVertexArray(gScreenTexture.VAO);
    glBindTexture(GL_TEXTURE_2D, gFrameBuffer.texColorBufferID);
...
int main(void)
{
    ...
    // have OpenGL not draw back-facing triangles (giving us a speedup!)
    //glEnable(GL_CULL_FACE);
    //glCullFace(GL_BACK); // options: GL_BACK (default), GL_FRONT, GL_FRONT_AND_BACK
    //glFrontFace(GL_CCW); // defines which order represents front facing: GL_CCW = counter-clockwise (default), GL_CW: clockwise
    ...
    //gDiffuseMap = createTexture("grass.png");
    ...
    while (!glfwWindowShouldClose(gWindow))
    {
        ...
        // press space to switch between drawing the screen texture as wireframe or regular
            ...
            gScreenTexture.renderMode = rectRenderModes[renderIndex];
        }
        ...
        //std::chrono::steady_clock::time_point pre_draw = std::chrono::steady_clock::now();
        draw();
        //std::chrono::steady_clock::time_point post_draw = std::chrono::steady_clock::now();

        //unsigned long long microseconds = 
        //    std::chrono::duration_cast<std::chrono::microseconds>(post_draw - pre_draw).count();
