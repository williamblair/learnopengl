...
vec3 calcDirectionalLight()
{
    ...
    // ambient
    vec3 ambient =  uDirLight.ambient * vec3(texture(uMaterial.diffuse, 
                                                  TexCoords));

    // diffuse
    float diff = max(dot(norm, lightDir), 0.0);
    vec3 diffuse = uDirLight.diffuse * diff * vec3(texture(uMaterial.diffuse,
                                                         TexCoords));
    ...
    vec3 specular = uDirLight.specular * vec3(texture(uMaterial.specular,
                                                    TexCoords))
                                              * spec;
...
{
    // ambient
    vec3 ambient =  uPosLight.ambient * vec3(texture(uMaterial.diffuse, 
                                                  TexCoords));
    ...
    // diffuse
    float diff = max(dot(norm, lightDir), 0.0);
    vec3 diffuse = uPosLight.diffuse * diff * vec3(texture(uMaterial.diffuse,
                                                         TexCoords));
    ...
    vec3 reflectDir = reflect(-lightDir, norm);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), uMaterial.shininess);
    vec3 specular = uPosLight.specular * vec3(texture(uMaterial.specular,
                                                    TexCoords))
                                        * spec;
...
{
    // ambient
    vec3 ambient =  uSpotLight.ambient * vec3(texture(uMaterial.diffuse, 
                                                  TexCoords));
    ...
    // diffuse
    float diff = max(dot(norm, lightDir), 0.0);
    vec3 diffuse = uSpotLight.diffuse * diff * vec3(texture(uMaterial.diffuse,
                                                         TexCoords));
    ...
    vec3 reflectDir = reflect(-lightDir, norm);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), uMaterial.shininess);
    vec3 specular = uSpotLight.specular * vec3(texture(uMaterial.specular,
                                                    TexCoords))
                                        * spec;
