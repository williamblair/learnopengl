...
// just used by normals display
ShaderProgram gNormalShaderProgram;
...
static void updateUniforms()
{
    ...
    gShaderProgram.SetVec3fv("uPosLight.specular", lightSpecular);
...
static void draw()
{
    ...
    // draw the model normals
    gNormalShaderProgram.Use();
    ...
    glm::mat4 projMat = glm::perspective(glm::radians(gCamera.FOV),
        ...
        nearClip,
        farClip);
    gNormalShaderProgram.SetMat4fv("uModel", gModelModelMat);
    gNormalShaderProgram.SetMat4fv("uView", viewMat);
    gNormalShaderProgram.SetMat4fv("uProjection", projMat);
    updateTransformationMatrix(gModelTransMat, gModelPosition, gCamera);
    gModel.Draw(gNormalShaderProgram);
}
...
int main(void)
{
    ...
    // For displaying the model normals
    gNormalShaderProgram.Create("vertexShader_normal.glsl", 
                                "fragmentShader_normal.glsl",
                                "geometryShader_normal.glsl");
