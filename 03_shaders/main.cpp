...
static void updateColorUniform()
{
    static std::string uniformName = "uVertexColor";
    static int uniformLoc = glGetUniformLocation(gShaderProgram.id,
                                                uniformName.c_str());
    float timeVal = glfwGetTime();
    float green = (sin(timeVal) / 2.0F) + 0.5F;
    glUniform4f(uniformLoc, 0.0F, green, 0.0F, 1.0F);
}
