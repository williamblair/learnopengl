...
Texture gAlbedoTex;
Texture gNormalTex;
Texture gMetallicTex;
Texture gRoughnessTex;
Texture gAoTex;
...
static void draw()
{
    static GLuint uAlbedoTex = glGetUniformLocation(gShaderProgram.id, "uAlbedoTex");
    static GLuint uMetallicTex = glGetUniformLocation(gShaderProgram.id, "uMetallicTex");
    static GLuint uRoughnessTex = glGetUniformLocation(gShaderProgram.id, "uRoughnessTex");
    static GLuint uAoTex = glGetUniformLocation(gShaderProgram.id, "uAoTex");
    static GLuint uNormalTex = glGetUniformLocation(gShaderProgram.id, "uNormalTex");
    ...
    glUniform1i(uAlbedoTex, 0); // gl textures 0 through 4
    glUniform1i(uMetallicTex, 1);
    glUniform1i(uRoughnessTex, 2);
    glUniform1i(uAoTex, 3);
    glUniform1i(uNormalTex, 4);

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, gAlbedoTex.id);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, gMetallicTex.id);
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, gRoughnessTex.id);
    glActiveTexture(GL_TEXTURE3);
    glBindTexture(GL_TEXTURE_2D, gAoTex.id);
    glActiveTexture(GL_TEXTURE4);
    glBindTexture(GL_TEXTURE_2D, gNormalTex.id);
...
int main(void)
{
    ...
    gAlbedoTex = createTexture("rusted_iron/albedo.png");
    gNormalTex = createTexture("rusted_iron/normal.png");
    gMetallicTex = createTexture("rusted_iron/metallic.png");
    gRoughnessTex = createTexture("rusted_iron/roughness.png");
    gAoTex = createTexture("rusted_iron/ao.png");
